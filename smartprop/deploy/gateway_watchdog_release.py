#!/usr/bin/env python3
"""Immutable source-only watchdog repair, preserving processes and sessions."""
import argparse
import base64
import hashlib
import json
...
MANIFEST = DEPLOY / 'gateway-watchdog-release-factory.toml'
CACHE = Path('/Users/vincent/.codex/artifact-cache/propertydemo-gateway-watchdog')
ARTIFACT = 'gateway_watchdog'
...
def package(base, output):
    ...
    code = (DEPLOY / 'gateway_watchdog_host.py').read_bytes()
    expected = remote(code.decode(), {'action': 'inspect'})['snapshot']
    with tempfile.TemporaryDirectory(prefix='smartprop-gateway-watchdog-') as temp:
        folder = Path(temp)
        ...
        (outputs / 'gateway_watchdog_host.py').write_bytes(code)
        (outputs / 'expected.json').write_bytes(canonical(expected))
        (outputs / 'watchdog.sh').write_bytes((ROOT / 'smartprop/scripts/openclaw-gateway-watchdog.sh').read_bytes())
        ...
        request = {'schema_version': 1,
                   ...
                   'outputs': [{'mode': 0o600, 'name': name, 'path': name}
                               for name in ('gateway_watchdog_host.py', 'expected.json', 'watchdog.sh')]}
...
def release(path, digest):
    ...
    with tempfile.TemporaryDirectory(prefix='smartprop-gateway-activate-') as temp:
        ...
        code = (output / 'gateway_watchdog_host.py').read_bytes()
        if code != (DEPLOY / 'gateway_watchdog_host.py').read_bytes():
            raise ValueError('host executor does not match source')
        expected = json.loads((output / 'expected.json').read_text())
        payload = (output / 'watchdog.sh').read_bytes()
        if payload != (ROOT / 'smartprop/scripts/openclaw-gateway-watchdog.sh').read_bytes():
            raise ValueError('watchdog artifact does not match source')
        result = remote(code.decode(), {'action': 'apply', 'expected': expected,
                         'payload': base64.b64encode(payload).decode(), 'payload_sha256': sha(payload)})
...
if __name__ == '__main__':
    ...
    try:
        if args.action == 'verify':
            run([sys.executable, 'smartprop/scripts/gateway-watchdog-verify.py'])
            result = {'status': 'passed', 'live': False}
        elif args.action == 'preflight':
            result = remote((DEPLOY / 'gateway_watchdog_host.py').read_text(), {'action': 'inspect'})
