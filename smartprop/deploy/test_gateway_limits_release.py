"""Offline integrity tests for the gateway memory-limit release driver."""
...
import gateway_limits_release as release_driver
...
class GatewayLimitsReleaseTest(unittest.TestCase):
    def plan(self):
        return {
            ...
            "deployment": {
                ...
                "required_smoke": [["smartprop/scripts/gateway-limits-verify.py"]],
                "artifacts": {"gateway_limits": ARTIFACT_DIGEST},
            },
        }
    ...
    def test_rejects_materialized_host_executor_that_differs_from_source_before_remote(self):
        ...
        def materialize(_cache, _digest, destination):
            destination.mkdir()
            (destination / "gateway_limits_host.py").write_bytes(b"changed executor")
    ...
    def test_preserves_failed_remote_rollback_result_without_reporting_success(self):
        ...
        source_executor = (release_driver.DEPLOY / "gateway_limits_host.py").read_bytes()

        def materialize(_cache, _digest, destination):
            destination.mkdir()
            (destination / "gateway_limits_host.py").write_bytes(source_executor)
