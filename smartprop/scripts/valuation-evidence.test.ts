...
function validEvidence() {
  return {
    ...
    sources: [
      ...
      {
        url: 'https://nexthome.sg/project/the-cliften',
...
describe('valuation source registry', () => {
  test('resolves canonical HTTPS hosts and allowed subdomains', () => {
    ...
    expect(resolveValuationSource('https://nexthome.sg/?ac=pc&pc=259342')).toMatchObject({
      canonicalUrl: 'https://nexthome.sg/?ac=pc&pc=259342',
    });
  });

  test('rejects non-HTTPS, credentials, ports, fragments, sensitive queries, look-alikes, and redirect wrappers', () => {
    for (const url of [
      'http://eservice.ura.gov.sg/a',
      'https://user:pass@eservice.ura.gov.sg/a',
      'https://user@eservice.ura.gov.sg/a',
      'https://:pass@eservice.ura.gov.sg/a',
      'https://eservice.ura.gov.sg:444/a',
      'https://eservice.ura.gov.sg/a#fragment',
      'https://eservice.ura.gov.sg/a?access_token=secret',
      'https://eservice.ura.gov.sg/a?api_key=secret',
      'https://eservice.ura.gov.sg/a?redirect=https://evil.example',
...
describe('validateValuationEvidence', () => {
  test('normalizes supported evidence with server-owned identity and deterministic hash', () => {
    ...
    const replayedLater = validateValuationEvidence(validEvidence(), {
      ...context,
      now: new Date(context.now.getTime() + 60_000),
    });
    ...
    expect(replayedLater).toEqual(first);
  ...
  test('rejects low confidence, inverted or absent values, and unsupported acquisition methods', () => {
    ...
    expectCode(() => validateValuationEvidence({
      ...validEvidence(), lowSgd: 1_000_000, midSgd: null, highSgd: 1_000_000,
    }, context), 'missing_midpoint');
    expectCode(() => validateValuationEvidence({
    ...
    expectCode(() => validateValuationEvidence({
      ...validEvidence(), midSgd: true,
    }, context), 'invalid_value');
    expectCode(() => validateValuationEvidence({
      ...validEvidence(), comparablesCount: true,
    }, context), 'invalid_comparables_count');
  });
