  ...
  try {
    ...
    setTimeout(async () => {
      try {
        ...
        await refreshLinkedInScheduler(0); // Start with retry count 0, function handles retries
    ...
    }, 12000); // Wait 12 seconds (after scraper scheduler) to reduce startup load
  } catch (error) {
