...
function optionalPositive(value: unknown, code: string, label: string): number | null {
  ...
  if (typeof value !== 'number' &&
    ...
    fail(code, `${label} must be numeric`);
  }
...
function sha256(value: unknown): string {
  ...
  return createHash('sha256').update(serialized ?? 'undefined', 'utf8').digest('hex');
}
...
export function validateValuationEvidence(
  ...
  if (midSgd === null && lowSgd !== null && highSgd !== null && lowSgd === highSgd) {
    fail('missing_midpoint', 'equal valuation endpoints require an explicit midpoint');
  }
  ...
  return { ...hashPayload, evidenceHash: sha256(hashPayload) };
}
