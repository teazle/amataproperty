...
const SENSITIVE_QUERY_PARAMETER = /(?:^|[_-])(access|api|auth|bearer|callback|continue|credential|key|password|redirect|return|secret|session|token|url|uri)(?:$|[_-])/i;
...
export function resolveValuationSource(urlValue: string): RegisteredValuationSource | null {
  ...
  if (urlValue.length > 2_000 || url.protocol !== 'https:' || url.username || url.password || url.port || url.hash) {
    return null;
  }

  for (const [name, value] of url.searchParams) {
    if (!name || name.length > 100 || value.length > 500 || SENSITIVE_QUERY_PARAMETER.test(name)) return null;
  }
